import Link from 'next/link';
import { prisma } from '@/lib/prisma';
import { getAdmin } from '@/lib/authorization';
import { AdminLoginForm } from '@/components/admin/AdminLoginForm';
import { AdminSuggestionClient } from '@/components/admin/AdminSuggestionClient';
import type { SuggestionKind } from '@prisma/client';

export const dynamic = 'force-dynamic';

export const metadata = {
  title: 'بررسی پیشنهادها - نظرمن',
};

const KIND_LABELS: Record<SuggestionKind, string> = {
  FIELD_CHANGE: 'اصلاح اطلاعات',
  CLOSED: 'این مکان بسته شده',
  MOVED: 'تغییر مکان',
  MISSING_INFO: 'اطلاعات ناقص است',
  PHOTO: 'پیشنهاد تصویر',
  GENERAL: 'سایر',
};

const FIELD_LABELS: Record<string, string> = {
  name: 'نام صفحه',
  description: 'معرفی',
  workingHours: 'ساعات کاری',
  imageUrl: 'تصویر',
  address: 'آدرس',
  scope: 'محدوده فعالیت',
  provinceSlug: 'استان',
  citySlug: 'شهر',
  latitude: 'عرض جغرافیایی',
  longitude: 'طول جغرافیایی',
};

type SuggestedTopic = {
  name: string;
  description: string | null;
  workingHours: string | null;
  imageUrl: string | null;
  address: string | null;
  province: { name: string } | null;
  city: { name: string } | null;
};

function currentValueFor(topic: SuggestedTopic, field: string): string {
  switch (field) {
    case 'name':
      return topic.name;
    case 'description':
      return topic.description ?? '';
    case 'workingHours':
      return topic.workingHours ?? '';
    case 'imageUrl':
      return topic.imageUrl ?? '';
    case 'address':
      return topic.address ?? '';
    case 'provinceSlug':
      return topic.province?.name ?? '';
    case 'citySlug':
      return topic.city?.name ?? '';
    default:
      return '';
  }
}

function formatPersianDate(value: Date): string {
  try {
    return value.toLocaleDateString('fa-IR', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
  } catch {
    return '';
  }
}

export default async function AdminSuggestionsPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string }>;
}) {
  const { status } = await searchParams;
  const selectedStatus =
    status === 'APPROVED' || status === 'REJECTED' || status === 'CHANGES_REQUESTED'
      ? status
      : 'PENDING_REVIEW';

  const suggestions = await prisma.suggestion.findMany({
    where: { status: selectedStatus },
    orderBy: { createdAt: 'desc' },
    take: 100,
    select: {
      id: true,
      kind: true,
      changes: true,
      note: true,
      status: true,
      decisionNote: true,
      createdAt: true,
      topic: {
        select: {
          id: true,
          slug: true,
          name: true,
          description: true,
          workingHours: true,
          imageUrl: true,
          address: true,
          province: { select: { name: true } },
          city: { select: { name: true } },
        },
      },
      author: {
        select: {
          displayName: true,
          phoneNumber: true,
        },
      },
    },
  });

  const admin = await getAdmin();

  if (!admin) {
    return <AdminLoginForm />;
  }

  const statusTabs = [
    { key: 'PENDING_REVIEW', label: 'در انتظار بررسی' },
    { key: 'CHANGES_REQUESTED', label: 'نیاز به اصلاح' },
    { key: 'APPROVED', label: 'تأیید شده' },
    { key: 'REJECTED', label: 'رد شده' },
  ];

  return (
    <main className="min-h-screen bg-paper pb-16">
      <div className="mx-auto w-full max-w-4xl px-5 py-10">
        <Link
          href="/admin"
          className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-turquoise-700 transition-colors hover:bg-turquoise-600/10"
        >
          بازگشت به مدیریت
        </Link>

        <header className="mt-6">
          <h1 className="font-display text-3xl text-ink-900">بررسی پیشنهادها</h1>
          <p className="mt-2 text-sm text-ink-600">
            پیشنهادهای کاربران برای اصلاح صفحه‌های منتشر شده
          </p>
        </header>

        <div className="mt-6 flex flex-wrap gap-2">
          {statusTabs.map((tab) => (
            <Link
              key={tab.key}
              href={`/admin/suggestions?status=${tab.key}`}
              className={`rounded-full px-4 py-2 text-[13px] font-bold transition-colors ${
                selectedStatus === tab.key
                  ? 'bg-turquoise-600 text-white'
                  : 'bg-white text-ink-600 ring-1 ring-ink-900/10 hover:bg-ink-900/5'
              }`}
            >
              {tab.label}
            </Link>
          ))}
        </div>

        {suggestions.length === 0 ? (
          <div className="mt-8 rounded-3xl bg-white p-6 text-sm text-ink-600 ring-1 ring-ink-900/[0.06]">
            پیشنهادی در این وضعیت وجود ندارد.
          </div>
        ) : (
          <div className="mt-8 space-y-4">
            {suggestions.map((suggestion) => {
              const changes =
                suggestion.changes && typeof suggestion.changes === 'object'
                  ? (suggestion.changes as Record<string, unknown>)
                  : {};

              return (
                <article
                  key={suggestion.id}
                  className="rounded-3xl bg-white p-5 ring-1 ring-ink-900/[0.06]"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h2 className="font-display text-lg text-ink-900">
                      {suggestion.topic.name}
                    </h2>
                    <Link
                      href={`/topic/${suggestion.topic.slug ?? suggestion.topic.id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[12px] font-medium text-turquoise-700 underline underline-offset-4"
                    >
                      مشاهده صفحه
                    </Link>
                  </div>

                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-ink-500">
                    <span className="rounded-full bg-turquoise-600/10 px-3 py-0.5 font-bold text-turquoise-700">
                      {KIND_LABELS[suggestion.kind]}
                    </span>
                    <span>
                      توسط {suggestion.author.displayName ?? suggestion.author.phoneNumber}
                    </span>
                    <span>{formatPersianDate(suggestion.createdAt)}</span>
                  </div>

                  {Object.keys(changes).length > 0 && (
                    <div className="mt-3 space-y-2 rounded-2xl bg-ink-900/[0.02] p-3 ring-1 ring-ink-900/[0.06]">
                      {Object.entries(changes).map(([field, proposed]) => (
                        <div key={field} className="grid gap-1 sm:grid-cols-2">
                          <div className="min-w-0">
                            <p className="text-[11px] font-bold text-ink-400">
                              فعلی — {FIELD_LABELS[field] ?? field}
                            </p>
                            <p className="break-words text-[13px] leading-6 text-ink-600">
                              {currentValueFor(suggestion.topic, field) || '—'}
                            </p>
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] font-bold text-turquoise-600">
                              پیشنهادی — {FIELD_LABELS[field] ?? field}
                            </p>
                            <p className="break-words text-[13px] leading-6 text-ink-800">
                              {String(proposed) || '—'}
                            </p>
                          </div>
                        </div>
                      ))}
                    </div>
                  )}

                  {suggestion.note && (
                    <p className="mt-3 whitespace-pre-line text-[13px] leading-7 text-ink-700">
                      {suggestion.note}
                    </p>
                  )}

                  {selectedStatus !== 'PENDING_REVIEW' && suggestion.decisionNote && (
                    <p className="mt-3 rounded-xl bg-saffron-50 p-3 text-[13px] leading-6 text-ink-700">
                      <span className="font-bold text-saffron-700">پیام مدیر: </span>
                      {suggestion.decisionNote}
                    </p>
                  )}

                  {selectedStatus === 'PENDING_REVIEW' && (
                    <AdminSuggestionClient suggestionId={suggestion.id} />
                  )}
                </article>
              );
            })}
          </div>
        )}
      </div>
    </main>
  );
}
